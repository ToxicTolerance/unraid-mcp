...
def register_vm_tools(mcp: FastMCP) -> None:
        ...
        query ListVMs {
          vms {
            ...
            domains {
              ...
              name
              state
              uuid # Deprecated but useful for correlation if needed
            }
          }
        }
        """
    ...
    async def manage_vm(vm_id: str, action: str) -> dict[str, Any]:
        """Manages a VM: start, stop, pause, resume, force_stop, reboot, reset. Uses VM ID (PrefixedID).
        ...
        """
        valid_actions = ["start", "stop", "pause", "resume", "forceStop", "reboot", "reset"]
    ...
    async def get_vm_details(vm_identifier: str) -> dict[str, Any]:
        """Retrieves detailed information for a specific VM by its ID, UUID or name.

        Args:
            vm_identifier: VM ID, UUID or name to retrieve details for
                ...
                if vms:
                    for vm_data in vms:
                        if (
                            ...
                            or vm_data.get("uuid") == vm_identifier
